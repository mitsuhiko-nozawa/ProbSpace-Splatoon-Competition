...
for iter in range(1, 2):
    ...
    train_fe_df = pd.DataFrame({'rf_feature_'+str(iter): train_df["pred"].values})
    train_fe_df.index.name = 'id'
    train_fe_df.to_csv('Train/train_rf_feature_{}.csv'.format(iter), index=False)
    test_fe_df = pd.DataFrame({'rf_feature_'+str(iter): preds})
    test_fe_df.index.name = 'id'
    test_fe_df.to_csv('Test/test_rf_feature_{}.csv'.format(iter), index=False)
