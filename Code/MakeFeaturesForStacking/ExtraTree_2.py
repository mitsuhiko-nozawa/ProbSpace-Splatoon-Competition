...
for col in ["category1", "category2", "subweapon", "special", "mainweapon", "reskin", "rank-mark"]:
    lbl = LabelEncoder()
    li = []
...
for col in [
    ...
    "team-mainweapon", "mode x team-mainweapon", "stage x team-mainweapon",
]:
    lbl = LabelEncoder()
    li = []
...
for iter in range(1, 2):
    ...
    train_fe_df = pd.DataFrame({'ert_feature_'+str(iter): train_df["pred"].values})
    train_fe_df.index.name = 'id'
    train_fe_df.to_csv('Train/train_ert_feature_{}.csv'.format(iter), index=False)
    test_fe_df = pd.DataFrame({'ert_feature_'+str(iter): preds})
    test_fe_df.index.name = 'id'
    test_fe_df.to_csv('Test/test_ert_feature_{}.csv'.format(iter), index=False)
